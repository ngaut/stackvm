...
from app.services.utils import StepType, parse_commit_message
...
class MySQLBranchManager(BranchManager):
    ...
    def _session(self):
        ...
        try:
            # Verify session is active with a simple query
            self._db_session.query(Branch).limit(1).all()
        except Exception as e:
            logger.info("Refreshing database session due to: %s", str(e))
            if self._db_session:
                self._db_session.close()
            self._db_session = SessionLocal()
        return self._db_session

    def _get_branch(self, branch_name: str) -> Optional[Branch]:
    ...
    def _get_commit(self, commit_hash: str) -> Optional[Commit]:
    ...
    def create_branch(self, branch_name: str) -> bool:
        """Create a new branch."""
        try:
            if self._get_branch(branch_name):
            ...
            if not self._current_commit_hash:
                ...
                self._session.add(initial_commit)
                self._session.flush()  # Get the commit hash
